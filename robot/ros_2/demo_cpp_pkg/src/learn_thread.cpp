...
int main()
{
    ...
    {
        std::cout << "Download finished:" << path << ", total" << result.length() << "contend:" << result << std::endl;
    };

    download.start_download("http://localhost:8000", "/place1.txt", download_finish_callback);
    download.start_download("http://localhost:8000", "/place2.txt", download_finish_callback);
    download.start_download("http://localhost:8000", "/place3.txt", download_finish_callback);
